...
Single-activity Jetpack Compose app (minSdk 24, targetSdk 36). No ViewModel, no Navigation Compose — screen state lives in `MainActivity.kt` via `rememberSaveable`. The data layer syncs races/teams/legend/member-tags via signed `GET` with ETag and uses Room as the single source of truth. NFC reader-mode and GPS-track recording are the two device-integration subsystems; the Карта tab renders track + taken КП over an offline per-race MBTiles basemap (MapLibre).
...
- **Dual-target upload loop** (marks + track + judge scans) — `Mutex.tryLock`-guarded, drains in `LIMIT` batches, marks only `accepted ∩ batch`, scoped by `(raceId, teamId)` (judge scans: `raceId` only — a judge station covers every team), idempotent by client UUID, flushes to both cloud and LAN targets independently. `MarkRepository.confirm` (single-mark confirm POST from the scan overlay) is the one marks POST **outside** `tryLock` — a running drain must never skip it.
- **Check method** — `isCounted` (`complete && (offline || confirmedAt != null)`, `data/marks/CheckMethod.kt`) replaces `complete` for scoring; `confirmedAt` is set only from the open scan overlay (`MarkRepository.confirm`), never by the background drain.
...
- `ui/scan/` — NFC take overlay (`ScanScreen`, 20 s window, completion hold, cloud/local confirm mode) + pure `ScanSession` state machine + pure `ScanFeedback` mapper + pure `ConfirmLoop`.
- `ui/marks/MarksScreen.kt` — Отметки tab: tile grid, metrics (live control-time/КВ cell, pure `ControlTime`), pre-start readiness checklist card (empty state; pure `ReadinessChecklist` model, device signals polled in `MainActivity`), photo tiles + lightbox, judge-review, hidden-КП and unconfirmed-take notices (dimmed tiles), take celebration.
...
- `data/db/` — Room **v8**, migrations 1→8 with committed schemas; local-only marks/track/bindings/judge-scan tables; DAO gotchas: preserve-on-resync, column-scoped updates, version-guarded flag flips, frame-drain queries.
...
- `data/MarkRepository.kt` — two-phase takes (tag `checkMethod` snapshot), dual-target upload loop, in-overlay `confirm`, photo-mark creation + frame drain, pure metric helpers; `trustedAt` seam backfills `trusted_ms` on upload for offline takes.
...
- `data/marks/` — pure `CheckMethod` + `isCounted`/`isUnconfirmed`, pure `PhotoPaths` codec (path-traversal guard, thumb convention), pure `PhotoTarget` router, `PhotoStorage` frame I/O adapter.
...
- **Dependencies (map tab)** — MapLibre Android SDK 13.6.1 (`org.maplibre.gl:android-sdk`); offline base via `mbtiles://<abs path>` raster source (`tileSize 256`), online fallback = OSM tiles (identifying User-Agent + visible attribution required). Its cache `mbgl-offline.db` is excluded from backup; maps live in `noBackupFilesDir/maps/`. `map_url` must be HTTPS (cleartext only for the LAN host).
- **Docs** — `docs/design/UI-NOTES.md` / `docs/design/DATA-NOTES.md` (per-file design notes, see above); `docs/API.md` (HMAC signing, ETag/304, 403 debug checklist); `docs/design/API.md` (legend crypto contract); `docs/design/UPLOAD.md` (track/marks/judge-scans upload contracts); `docs/mobile-admin-auth-and-tags.md` (admin auth + tags); `docs/IOS-PARITY.md` (iOS features not yet ported to Android, each pointing to its iOS plan spec; remove an item when it ships); `assets/template_v2.html` (hi-fi M3 mockup — screens A1b/A2/A3/A4).
- **M3 tokens** (in `Color.kt`) — `OrangeCta #C65A2E` (FABs, active nav, NFC tint), `tertiary #1F7A3D` (success/taken), `inverseSurface #1D242D` (dark hero cards), `outlineVariant #E2E6EB` (dividers), `WarningAmber #B26A00` / `WarningAmberDark #FFB951` (readiness-checklist `Warning` status only; picked via `isDarkScheme()`; the Отметки notices keep the single `errorContainer` alert palette).
