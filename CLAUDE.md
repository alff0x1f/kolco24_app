...
- `data/db/` — Room v1 (`AppDatabase`, `exportSchema = false`, no migrations, no `fallbackToDestructiveMigration` — a schema version bump without a migration will crash on upgrade): `RaceEntity` (table `races`, server `id` PK — also the domain model, no third layer), `SyncMetaEntity` (table `sync_meta`, composite PK `(origin, resource)`, stores ETag per origin). `RaceDao.replaceAll` is a `@Transaction` deleteAll+insertAll. DAOs are not unit-tested (trivial queries).
