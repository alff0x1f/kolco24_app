...
- `ui/map/` — Карта tab: `MapScreen` (download/progress cards, OSM attribution), `TrackMapView` MapLibre adapter (`settledPage == 2` gate, bitmap pins, OSM UA), pure `MapLogic`/`MapAvailability`.
...
- `data/map/` — race MBTiles basemaps: `MapFileStorage` (`noBackupFilesDir/maps/`, file = «downloaded» flag, `.part` → validate → rename), `MapDownloader` (plain unsigned OkHttp), `MapRepository` (single app-wide download on `applicationScope`, `downloaded` flow), pure `MbtilesMetadata` + SQLite `MbtilesSqlite` readers.
