 ...
 * is POSTed to [UploadTarget] and only a server accept counts the КП. Pure so the loop is JVM-tested.
 */
...
/**
 * Retry [attempt] until it returns [UploadResultKind.Ok] ([ConfirmState.Confirmed]) or [timeoutMs]
 * has elapsed since the start ([ConfirmState.Failed], `offline` from the last result). An attempt that
 * starts before the deadline and succeeds after it still confirms — the deadline is checked only after
 * a failed attempt. Cancellation propagates normally (no terminal state is emitted).
 */
suspend fun runConfirm(
    ...
    while (true) {
        ...
        val r = attempt()
        if (r == UploadResultKind.Ok) {
            onState(ConfirmState.Confirmed)
            return
        }
        if (elapsedNow() - start >= timeoutMs) {
            onState(ConfirmState.Failed(offline = r == UploadResultKind.Offline))
            return
        }
        delay(retryMs)
        n++
