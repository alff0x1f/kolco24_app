package ru.kolco24.kolco24.ui.theme

import androidx.compose.ui.graphics.Color

// Light scheme — seeded from brand red #B3261E
val BrandRed            = Color(0xFFB3261E)
val OnBrandRed          = Color(0xFFFFFFFF)
val BrandRedContainer   = Color(0xFFFFDAD5)
val OnBrandRedContainer = Color(0xFF410002)

val Secondary            = Color(0xFF974742)
val OnSecondary          = Color(0xFFFFFFFF)
val SecondaryContainer   = Color(0xFFFFDAD6)
val OnSecondaryContainer = Color(0xFF3B0909)

val Tertiary             = Color(0xFF1F7A3D)
val OnTertiary           = Color(0xFFFFFFFF)
val TertiaryContainer    = Color(0xFFA6F2BC)
val OnTertiaryContainer  = Color(0xFF002109)

val SurfaceLight              = Color(0xFFEEF0F3)
val OnSurfaceLight            = Color(0xFF161A1F)
val OnSurfaceVariantLight     = Color(0xFF6B7785)
val SurfaceContainerLowest    = Color(0xFFFFFFFF)
val SurfaceContainerLow       = Color(0xFFFFFFFF)
val SurfaceContainerDefault   = Color(0xFFFFFFFF)
val SurfaceContainerHigh      = Color(0xFFF4F6F9)
val SurfaceContainerHighest   = Color(0xFFE8ECF1)

val OutlineLight        = Color(0xFF6B7785)
val OutlineVariantLight = Color(0xFFE2E6EB)

val InverseSurface   = Color(0xFF1D242D)
val InverseOnSurface = Color(0xFFF4F6F9)

val OrangeCta = Color(0xFFC65A2E)
val InversePrimary   = Color(0xFFFFB4AB)

// Checkpoint color-bar palette (legend leading bar). Saturated single shade,
// same in light & dark. Green reuses Tertiary, orange reuses OrangeCta.
val CpColorRed    = Color(0xFFE53935)
val CpColorBlue   = Color(0xFF1E88E5)
val CpColorYellow = Color(0xFFF4B400)
val CpColorPurple = Color(0xFF8E44AD)

val ErrorLight      = Color(0xFFBA1A1A)
val OnErrorLight    = Color(0xFFFFFFFF)
val ErrorContainer  = Color(0xFFFFDAD6)
val OnErrorContainer = Color(0xFF410002)

// Dark scheme
val InverseSurfaceDark   = Color(0xFFEEF0F3)
val InverseOnSurfaceDark = Color(0xFF1D242D)

val BrandRedDark            = Color(0xFFFFB4AB)
val OnBrandRedDark          = Color(0xFF690005)
val BrandRedContainerDark   = Color(0xFF93000A)
val OnBrandRedContainerDark = Color(0xFFFFDAD5)
val SurfaceDark             = Color(0xFF201A19)
val OnSurfaceDark           = Color(0xFFEDE0DD)
val OnSurfaceVariantDark    = Color(0xFFD8C2BE)
val SurfaceContainerLowestDark  = Color(0xFF171312)
val SurfaceContainerLowDark     = Color(0xFF2B2422)
val SurfaceContainerDefaultDark = Color(0xFF322A28)
val SurfaceContainerHighDark    = Color(0xFF3D3330)
val SurfaceContainerHighestDark = Color(0xFF493D39)
val OutlineDark            = Color(0xFFA18C88)
val OutlineVariantDark     = Color(0xFF4F403D)
val TertiaryDark            = Color(0xFF83D99B)
val OnTertiaryDark          = Color(0xFF003919)
val TertiaryContainerDark   = Color(0xFF005228)
val OnTertiaryContainerDark = Color(0xFFA6F2BC)
