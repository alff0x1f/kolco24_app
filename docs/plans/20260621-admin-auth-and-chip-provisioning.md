...
- [x] `HorizontalPager` over `legendRepository.checkpointsForRace(raceId).collectAsState()` for the selected team's race; if no team/race selected, show a hint (default to selected race — keep tight). State which checkpoint types are pageable — bind accepts only real КП (server `404`s on `hidden`), so either filter the pager to bindable rows or accept the inline `404`/`400` as the guard; note the choice. **Choice: no client-side type filter — page over all checkpoints and let the server be authoritative; a non-bindable tap surfaces inline as `404`/`400` in the scan zone (forward-compatible, keeps it tight).**
- [x] Pre-seed per-КП bound counts from cached `TagEntity` rows (count per `checkpoint.id`) via a Room flow / one-shot read. **Added `LegendRepository.tagsForRace(raceId): Flow<List<TagEntity>>`; grouped by `point` (`eachCount()`).**
- [x] Add the `@Volatile var onTagForProvision: ((Tag) -> Unit)?` hook to `MainActivity` and splice it into the `onTagDiscovered` priority chain right after `onTagForWrite` (per Technical Details). `DisposableEffect` arms `(context as MainActivity).onTagForProvision = { tag -> ... }`; clears to null on dispose. Bind only when `!pagerState.isScrollInProgress`, to `checkpoints[pagerState.currentPage]`. (Arming `DisposableEffect` lives in `ProvisioningScreen`, keeping host changes minimal; `rememberUpdatedState(cps)` lets the long-lived hook read the latest checkpoints without re-arming.)
- [x] On tag: `apiClient.bindTag(raceId, cp.id, normalizeNfcUid(tag.id))` (already runs on `Dispatchers.IO` inside `post`); `Success` → `chipCodeFromHex(code)` → `writeChipCodeNdef(tag, bytes, BuildConfig.APPLICATION_ID)` off-main; update `ProvisionState` + rack + rail. (Exposed `AppContainer.apiClient` publicly — no provisioning repo, the bind response isn't persisted.) Used `rememberCoroutineScope` for the UI-driving launch, mirroring the debug `WriteChipDialog`: the chip must stay on the phone for the write, so the op completes within the open overlay.
- [x] Error handling: `409` → generic «уже привязан к другому КП» (no КП number in the body), not added; `403/404/400/429/Offline` → inline scan-zone message via `provisionErrorMessage`; `401` → `adminAuthRepository.onUnauthorized()` + `onClose()` (drops to login); write failure → «Не удалось записать, приложите снова» (re-scan re-binds idempotently `200` + same `code` + rewrite; state-guarded so an in-flight tap is ignored, a settled Success/Failed accepts a re-tap).
- [x] Add a `BackHandler` for the provisioning sub-state (host flag `showProvisioning`); reset on overlay close, `onScanClick`, and team switch. (Admin's `BackHandler` guarded with `!showProvisioning`; provisioning's is registered after it so it wins when stacked.)
- [x] Compile + run unit suite — must pass before Task 13. (Pager UI untested; logic via Task 11 tests.)
