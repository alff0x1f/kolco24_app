...
- **Тёплый старт через boot identity (P0):** `verified = persisted != null && currentBoot != null && persisted.bootCount == currentBoot` — **оба** `bootCount` non-null **и** равны. Кейс `null == null` **не** даёт тёплый старт (иначе verified был бы ошибочно `true`). Иначе (reboot, либо любой `null`) ⇒ `verified = false`.
- `trusted(): Long?` `= serverEpochMs + (elapsedNow − anchorElapsedMs)` — non-null только при `verified`. **Reboot-детект на чтении:** инвалидировать, только если `anchor.bootCount != null && currentBoot != null && различны`; если любой `null` ⇒ fallback `elapsedNow < anchorElapsedMs`. `null` **никогда** не считается доказательством reboot/новой сессии.
- **Правило приёма якоря (P1 out-of-order + P2 null):** `onServerTime(serverMs, anchorElapsed, wallNow, bootNow)` принимает, если: нет текущего; **или** оба `bootCount` non-null и **различны** (новая сессия); **или** (та же сессия, либо любой `null`) `anchorElapsed >= current.anchorElapsedMs` (новее по монотонике). Поздний ответ с меньшим `anchorElapsed` в той же сессии **отбрасывается**. `bootNow == null` **не** трактуется как «новая сессия всегда побеждает» — падает в монотонную ветку, чтобы случайный `null` не понизил хороший якорь.
...
- [ ] создать `TrustedClock.kt` с `ClockAnchor` (+`bootCount`), `TimeSample` (+`bootCount`), `ClockStatus` (sealed; `Ok` без payload), приватным `ClockState`, `SKEW_THRESHOLD_MS = 60_000L`
- [ ] конструктор: `elapsedProvider: () -> Long`, `wallProvider: () -> Long`, `bootCountProvider: () -> Int?`, `persist: (ClockAnchor) -> Unit = {}`, `persisted: ClockAnchor? = null`
- [ ] состояние — `AtomicReference<ClockState>` для lock-free чтений; **записи под `synchronized(lock)`**: в одном критическом участке прочитать текущий state → применить правило приёма → при accept по порядку `ref.set(new)` → `persist(anchor)` → `statusFlow.value = …` (сериализация писателей, P1)
- [ ] **тёплый старт (P0, null-safe):** `verified = persisted != null && currentBoot != null && persisted.bootCount == currentBoot` (где `currentBoot = bootCountProvider()`) — `null == null` тёплый старт **не** включает
- [ ] чистый helper `computeTrusted(state, elapsedNow, bootNow): Long?` (без побочек/`.get()`); `trusted()` и `sample()` дёргают его на **уже снятых** значениях. Reboot-инвалидация: только `anchor.bootCount != null && bootNow != null && различны`; иначе fallback `elapsedNow < anchorElapsedMs`; `null` не считается reboot
...
- [ ] хелпер форматирования `skewMs → «N мин»` — чистая функция: **по модулю** (`ceil(abs(skewMs) / 60_000.0)`), т.к. текст без направления — отстающие часы дают отрицательный skew, нельзя «−2 мин». **Не** вызывать `abs(Long.MIN_VALUE)` напрямую (UB — остаётся отрицательным): брать `abs` через `Math.abs` с предварительным клампом или считать в `Double` (`abs(skewMs.toDouble())`). Мини-тест: **оба знака** (+90 000 и −90 000 → «2 мин»), 60 001 → «1 мин», 119 000 → «2 мин», `Long.MIN_VALUE` не падает/не даёт отрицательное
