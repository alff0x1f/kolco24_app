...
- [ ] write tests `TrackRepositoryTest` (fake `TrackDao`): `insertAll` мапит и пишет, `deleteForTeam` чистит, метрики/счётчик корректны, `trustedMs` досчитывается из `elapsedRealtimeAt` (fake clock), `bootCount`/`id` берутся из инжектированных провайдеров (детерминированный тест)
- [ ] ⚠️ **(из ревью) write test батча из ≥2 точек с РАЗНЫМИ `elapsedRealtimeNanos`:** проверить, что каждая получает **свой** `wallMs = wallNow + (elapsedAt − elapsedNow)` (точки различаются по wall-времени ровно на Δelapsed), а не одинаковое время вставки пачки — это и есть критерий «время точки корректно при батчинге» для wall-fallback
