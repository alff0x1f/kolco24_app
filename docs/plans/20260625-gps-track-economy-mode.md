...
- [x] verify: default install records on `Precise` (15 s) unchanged; toggling Switch to economy persists and survives process death; mid-recording toggle live-restarts the engine without a track gap — logic review confirms `activeProfile = container.trackProfilePreference.profile.value` at start (default `Precise` from `parseTrackProfile(null)`), persistence via `TrackProfilePreference` (covered by `TrackProfilePreferenceTest`), and the live-restart path (`onStartCommand` lines 130–147); on-device track-gap check is Post-Completion
- [x] **logic review — Stop fix (critical, Service untested):** verified `teardown()` uses `flushThen(e) { if (engine === e) { e.stop(); engine = null }; finishTeardown() }` (lines 197–204) — identity-guarded, exactly-once via `AtomicBoolean`, with the `FLUSH_TIMEOUT_MS` (4 s) `postDelayed` fallback (lines 211–219); buffer enqueued to `applicationScope` before stop (Fused `flushLocations().addOnCompleteListener`)
- [x] **logic review — leaked-timeout cleanup (re-review #1, critical):** verified `finishTeardown()` nulls `engine` **and** `mainHandler.removeCallbacksAndMessages(null)` **before** `serviceScope.cancel()`/`stopSelf()` (lines 229–245), so a pending live-switch `flushThen` timeout can't `startEngine` a zombie after teardown
- [x] **logic review — live-apply (critical):** verified `profileJob` is cancel-then-relaunched inside `onStartCommand` (lines 130–131, not once); runs on `Dispatchers.Main.immediate`; uses `activeProfile` + `collectLatest` **without `.drop(1)`**; restart body is `flushThen(e) { if (engine !== e) return@flushThen; e.stop(); restart to latest .value }` (lines 139–145); `profileJob` cancelled in `finishTeardown()` (line 233)
- [x] **logic review — onDestroy (review #4):** verified a bare `onDestroy()` (no `ACTION_STOP`) routes through best-effort `flushThen` reusing `finishTeardown()` (lines 174–189)
- [x] verify the guarantee wording is honest: «Стоп» + live-switch are lossless (buffer enqueued before stop via `flushThen`); a bare `onDestroy()` or a **hard kill** may lose up to `maxDelay` — and Precise stays 60 s to bound that. Durability is via `applicationScope` survival (lines 163–165), not a synchronous write barrier — confirmed in code + KDoc
- [x] verify edge cases: unknown stored value → `Precise` (`parseTrackProfile` fallback, `TrackProfileTest`); profile change while **not** recording just persists (no running `profileJob`, applied at next start's `.value` read); rapid double-toggle settles on the **latest** profile via restart-to-`.value` (line 142) with `activeProfile`-skip suppressing only last-completed repeats; a change landing between the initial `.value` read and subscription is **not** dropped (no `.drop(1)`)
