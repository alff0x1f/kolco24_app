...
- [ ] Add `photosUploadedLocal: Boolean = false` and `photosUploadedCloud: Boolean = false` to `MarkEntity` (KDoc: per-mark frame-upload flags per target; default false; metadata-first ordering)
- [ ] Bump `@Database(version = 3)` in `AppDatabase.kt`
- [ ] Add `MIGRATION_2_3` (two `ALTER TABLE marks ADD COLUMN … INTEGER NOT NULL DEFAULT 0`) with KDoc; append to `.addMigrations(MIGRATION_1_2, MIGRATION_2_3)`
- [ ] Build to emit `schemas/3.json`; commit it
- [ ] Extend `MigrationTest`: `MIGRATION_2_3` adds both columns with default 0 and preserves existing rows
- [ ] Run `./gradlew assembleDebug` + `connectedDebugAndroidTest` (MigrationTest) — must pass before Task 2
