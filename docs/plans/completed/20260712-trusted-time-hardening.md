...
- **GPS-кандидат**: `RawFix` (`data/track/TrackModels.kt`) расширяется полями
  ...
  2-D — нет). Дешёвая эвристика, доступная с minSdk 24; **сужает** (не закрывает) вектор, сохраняя
  офлайн-якорь на GMS. Остаточный риск E2 + availability-gap E1 — в «Осознанные ограничения».
...
- **Backfill не спасает строку, чьё первое сетевое касание — её собственная успешная выгрузка
  (codex-обзор, findings 1/2).** `backfillTrustedMs` в `MarkRepository`/`JudgeScanRepository`
  вычисляет `trusted_ms` в момент сборки DTO — **до** POST. Если облачные часы устройства
  были `NoSync` (офлайн с загрузки) и именно ответ этой выгрузки первым ставит якорь
  (`ServerTimeInterceptor` читает `Date` из ответа облачного `apiClient`), то DTO уже ушёл с
  `trusted_ms == null`, а `markUploaded*` делает строку неотбираемой для повторной отправки
  на **этот** таргет — значит серверный fill-if-null для неё уже не сработает (нечем: клиент
  больше не пришлёт эту строку с непустым `trusted_ms`). fill-if-null помогает только строкам,
  выгруженным **после** появления якоря (и второму таргету при dual-target, если якорь встал
  между двумя выгрузками одной строки). **Почему не чиним на клиенте:** единственный способ
  заполнить `trusted_ms` до сборки DTO — заякорить часы до выгрузки, а это либо блокирует/
  задерживает выгрузку (нельзя — потеря данных при чисто офлайн-гонке без LAN-времени), либо
  вводит хрупкую связку «не помечать выгруженным, если POST только что поставил якорь» (риск
  циклов ре-аплоада, отвергнуто). На практике якорь почти всегда ставит `/sync/`-GET на старте
  (Launch A/B) **раньше** любой выгрузки; окно гонки — только «офлайн с загрузки, и самый первый
  успешный сетевой обмен — это ровно upload». **Бэкстоп — серверная фрод-проверка** «старт/финиш
  (и любая отметка) с `trusted_ms == null`» уже в Post-Completion: такая строка и так помечается
  подозрительной, что и есть искомая защита.
- **Fused-провайдер и подделка wall-часов (Finding E, review iter2).** На GMS-устройстве
  ...
  легитимный облачный якорь. **Сужено в `gpsTimeCandidate.isSatelliteBacked`:** `"gps"` доверяем
  по имени провайдера (явный `GPS_PROVIDER` — гарантия ОС о спутниковом источнике); `"fused"`
  принимаем **только** при `altitude != null` — чистый GNSS-фикс 3-D и несёт высоту, чистый
  WiFi/cell 2-D — нет. Дешёвая эвристика, доступная с minSdk 24 (`Location.hasAltitude()`;
  `verticalAccuracyMeters` сильнее, но API 26+ — не требуем, иначе якорь мёртв на API 24/25).
  Это **эвристика, поднимающая планку**, а не гарантия — она несовершенна в обе стороны
  (Finding E1/E2, review iter3), и обе стороны — осознанный консервативный компромисс, не баг:
  - **(E2, безопасность — false positive.)** `altitude != null` **не доказывает**, что `time`
    спутниковый: `FusedLocationProviderClient` — чёрный ящик, на телефоне с барометром или примешав
    закешированную GNSS-высоту к текущему WiFi-горизонтальному фиксу, он может выдать `altitude != null`
    при `time`, равном подделываемым wall-часам. Гейт **сужает**, но не устраняет вектор подделки;
    реальная последняя линия — серверные фрод-проверки (Post-Completion), тот же потолок клиентской
    защиты, что и первый пункт.
  - **(E1, доступность — false negative.)** 2-D-only GNSS-фикс (лок по 3 спутникам под плотным
    пологом леса — ровно тот офлайн-сценарий, на который рассчитан якорь) несёт валидное спутниковое
    `time`, но `hasAltitude() == false`, поэтому на GMS (`provider = "fused"`) он консервативно
    отклоняется — GPS-якорь там может быть недоступен и часы падают в `NoSync` (безопасно). Не чиним
    ослаблением гейта: это вернуло бы вектор E2 с WiFi-фиксами. Для гейта безопасности отклонить
    неоднозначный легитимный фикс (→ `NoSync`) безопаснее, чем принять подделываемый (→ ложный якорь).
- **Порядок `syncLanTime` перед `/sync/`-манифестом (codex-обзор iter2).** `X-App-Ts` штампуется
  из доверенного времени (`TrustedClock.signingSeconds()`), поэтому перекошенные `NoSync`-часы
  `403`-ят каждый freshness-проверяемый подписанный запрос, включая сам `/sync/`-манифест, а
  `/app/time/` от окна ±300 с освобождён. Если бы `syncLanTime` вызывался **после** решения по
  манифесту, первый `enterLocalMode`/`probeLocalAndRenew` на перекошенном устройстве получал бы
  `manifest == null` → `LocalUnreachable`/`Keep`, а якорь ставился бы слишком поздно — только для
  следующей попытки. Поэтому `syncLanTime` вызывается **первым** (вне leaseMutex, до `fetchSync`):
  reachable-LAN перекошенное устройство якорит часы и пиннит с **первого** раза, а действительно
  недоступный LAN остаётся дешёвым no-op → манифест всё равно падает → тот же исход, что и раньше.
  Остаточный край — облачно-только устройство без доступного LAN: его первый `/sync/`-GET на
  холодном старте лечится однократным ретраем `AppSignatureInterceptor` после ре-якоря по `Date`
  из ответа `403`.
